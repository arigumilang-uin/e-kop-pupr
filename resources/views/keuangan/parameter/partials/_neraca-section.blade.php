<div class="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
    <div class="bg-stone-50 border-b border-stone-200 px-6 py-4">
        <h3 class="font-black text-stone-800 uppercase tracking-widest text-sm">{{ $title }}</h3>
    </div>
            ...
            <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 group hover:bg-stone-50/50 transition-colors">
                <div class="flex-grow">
                    <div class="flex items-center gap-3 mb-1">
                        ...
                        @if($item->is_pengurang)
                            <span class="px-2 py-0.5 bg-red-100 text-red-700 text-[10px] font-black uppercase rounded-md tracking-wider">Pengurang</span>
                        @endif
                    </div>
                    @if($item->isOtomatis())
                ...
                <div class="flex items-center gap-4 sm:min-w-[300px] justify-between sm:justify-end">
                    <div class="text-right">
                        @if($item->isOtomatis() && $item->nominal_manual > 0)
                            <p class="text-[10px] text-stone-400 font-medium mb-0.5">Saldo Awal Manual:</p>
                        @endif
                    ...
                    <div>
                        <button @click="$dispatch('open-modal', 'modal-edit-neraca-{{ $item->id }}')" class="w-8 h-8 rounded-lg bg-stone-100 text-stone-600 flex items-center justify-center hover:bg-[#043d2e] hover:text-white transition-colors" title="Edit Parameter">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                        </button>
                        
                        <form action="{{ route('keuangan.parameter.neraca.destroy', $item) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus parameter ini? Nilai total pada Neraca tahun ini akan ikut berubah.');">
                            @csrf
                            @method('DELETE')
