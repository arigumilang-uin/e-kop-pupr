            ...
            <x-table.tr>
                <x-table.td>
                    <div class="flex flex-col">
                        <button type="button" @click="openModal({{ $anggota->id }}, '{{ addslashes($anggota->nama) }}', {{ $anggota->neto_pokok > 0 ? 'true' : 'false' }})" class="font-bold text-stone-800 hover:text-[#043d2e] transition-colors flex items-center gap-1.5 w-fit text-left focus:outline-none" title="Catat Simpanan Manual">
                            {{ $anggota->nama }}
